				...
				if (!ParaInfo.supportsService("com.sun.star.text.TextTable")) {
						...
						if (!weightRaised) {
							if (hasHeadings) {
								if (!(CurrentPortionWeight == GOST.HeadingWeight)&&(!CurrentPortionText.equals(""))) {
									int dotcount = CurrentPortionText.length() - CurrentPortionText.replaceAll(".","").length();
									if (dotcount < 2) {
					...
					if (!portionText.equals("")){
						...
						if (hasHeadings) {
							...
							if (portionText.equals("РЕФЕРАТ")||portionText.equals("ВВЕДЕНИЕ")||portionText.equals("ЗАКЛЮЧЕНИЕ")||portionText.equals("СПИСОК ИСПОЛЬЗОВАННЫХ ИСТОЧНИКОВ")) {
							...
							if (!(CurrentParaAdjust == GOST.Adjust)) {
								//if it isn't justified
								if (!portionText.contains("Рисунок")) {
									//if it isn't an image description
									String adjustVal = "";
					                adjustVal += CurrentParaAdjust;
									switch (adjustVal){
										case ("0"): adjustVal = "по левому краю"; break;
									...
									if (CurrentParaAdjust != 3) {
										...
										paraMistakes += "Выравнивание: " + adjustVal + ", для подписей ожидается выравнивание по центру\n";
										raised = true;
									}
								}
							}
